...
char* DSM::spin_on(GlobalAddress lock_addr) {
  uint64_t *spin_loc = (uint64_t *)((uint64_t) lockMetaAddr + (lock_addr.nodeID * conf.lockNR * 8) + lock_addr.offset);

  GlobalAddress ga;
  ga.nodeID = lock_addr.nodeID;
  ga.offset = (lock_addr.nodeID * conf.lockNR * 8) + lock_addr.offset;
  cerr << "SPINNING ON: " << ga << "\n\n";
