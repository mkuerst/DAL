...
def plot_MC_rlocks(DATA, comm_prot="rdma", remote_locks=["spinlock"], num_clients=[1,4], nt=16):
    fig_empty_mc, ax_empty_mc, ax_empty_mc2 = make_ax_fig(FIG_X, FIG_Y)
    fig_empty_mc_fair, ax_empty_mc_fair, ax_empty_mc_fair2 = make_ax_fig(FIG_X, FIG_Y)
    ...
    fig_mem_mc_h, ax_mem_mc_h, ax_mem_mc2_h = make_ax_fig(FIG_X, FIG_Y)
    bw = 0.9 / len(num_clients)
    client_offsets = make_offset(num_clients) 
    ...
    lat_inc = ["gwait_acq", "lwait_acq"]
    clients = []
    # add_vline([ax_empty_mc, ax_empty_mc_fair, ax_mem_mc, ax_mem_mc_h], 0, 0.3)
    for impl in IMPL:
        for remote_lock in remote_locks:
            if not impl in DATA[comm_prot][remote_lock]["client"]["cum"]["empty_cs2n"].keys():
                continue
            if DATA[comm_prot][remote_lock]["client"]["cum"]["empty_cs2n"][impl] == {}:
                continue
            position += 1
            x_positions.append(position)
            x_labels.append(impl+f"_{remote_lock}")

            # add_vline([ax_empty_mc, ax_empty_mc_fair, ax_mem_mc, ax_mem_mc_h], position, 0.3)
            DE = DATA[comm_prot][remote_lock]["client"]["cum"]["empty_cs2n"][impl]
            DM2N = DATA[comm_prot][remote_lock]["client"]["cum"]["mem2n"][impl]
            for nclients in num_clients:
                ...
                for nthreads, values in DE[nclients].items():
                    if nthreads == nt:
                        duration = values["total_duration"].max() * DURATION_FACTOR 
                        add_lat(ax_empty_mc, ax_empty_mc2, values, position+client_offsets[nclients], comm_prot, bw, lat_inc)
                        add_box(ax_empty_mc_fair, ax_empty_mc_fair2, position+client_offsets[nclients], values)
                        la = values["lock_acquires"].mean() / duration
                        # maxs[nclients] = la if maxs[nclients] < la else maxs[nclients]
                ...
                for nthreads, values in DM2N[nclients].items():
                    if nthreads == nt:
                        values = values[values["array_size"] == values["array_size"].max()]
                        duration = values["total_duration"].max() * DURATION_FACTOR 
                        add_lat(ax_mem_mc, ax_mem_mc2, values, position+client_offsets[nclients], comm_prot, bw, lat_inc)
                        add_box(ax_mem_mc_fair, ax_mem_mc_fair2, position+client_offsets[nclients], values)
                        la = values["lock_acquires"].mean() / duration
                        # maxsm[nclients] = la if maxsm[nclients] < la else maxsm[remote_lock]

                        add_lat(ax_mem_mc_h, ax_mem_mc2_h, values, position+client_offsets[nclients], comm_prot, bw, ["lock_hold"])
...
plot_MC_rlocks(DATA, remote_locks=["spinlock", "lease1"], num_clients=[1,4], nt=16)