...
REMOTE_NODES=r630-08,r630-09,r630-10,r630-11,r630-12
cleanup_exit() {
    echo ""
    echo "Cleaning up..."
    pkill -P $$ 
    ...
    clush --hostfile ./sysg_nodes.txt "rdma resource show mr | awk '{print \$12}' | sort -u | xargs -r kill -9"> /dev/null 2>&1
    clush --hostfile ./sysg_nodes.txt "rdma resource show mr | awk '{print \$16}' | sort -u | xargs -r kill -9"> /dev/null 2>&1
                ...
                do
                    ...
                    do
                        ...
                        for ((run = 0; run < runNR; run++)); do
                            ...
                            clush --hostfile <(head -n $nodeNR ./sysg_nodes.txt) \
                                "$mb_exe \
                                -t $threadNR \
                                -d $duration \
                                -m $mode \
                                ...
                                -z $zipfan"
                                # 2>> $log_file"
                            # "LD_PRELOAD=$cn_so $disa_bench -t $i -d $duration -s $server_ip -p $p_ips -m $j -c $ncns -f $cn_rescum_file -g $cn_ressingle_file -l $nlocks -r $runs -e $mem_runs"
                            # 2>&1
