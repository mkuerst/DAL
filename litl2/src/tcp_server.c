...
__thread char released_msg[BUFFER_SIZE];
...
/*
==========================================================
TCP SPINLOCK MT - NEEDED TO MEASURE PER CLIENT COMM TIME
==========================================================
*/
void *run_client_lock_impl(void *_arg)
{
    ...
    while (num_connections < nclients) {
        ...
        for (int i = 0; i < event_count; i++) {
            if (events[i].data.fd == server_fd) {
                ...
                char buf[32] = {0};
                int bytes_read = recv(client_fd, buf, 32, 0);
                if (bytes_read < 0) {
                    tcp_client_error(client_fd, "Server failed at receving client_id\n");
                }
                int client_id;
                if (sscanf(buf, "%d", &client_id) == 1) {
                    DEBUG("Received client_id %d\n", client_id);
                    if (send(client_fd, buf, 32, 0) < 0)
                ...
                clients[client_id] = (client_data) {0};
                clients[client_id].sockfd = client_fd;
                clients[client_id].mode = mode;
                clients[client_id].id = client_id;
                pthread_create(&clients[client_id].thread, NULL, run_client_lock_impl, &clients[client_id]);
            } 
        }
        num_connections++;
    }
...
==========================================
*/
//TODO: measuring comm time w/ multiple clients requires spawning a thread per client
//TODO: Timestamp comm time measurements don't work yet
// void *run_rspin(void *_arg) {
//     client_data *client = (client_data *) _arg;
// }
